            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="align-middle text-center pr-3">
                        ...
                        @if($venta->estado !== 'ANULADO')
                        <form id="form-anular-{{ $venta->id }}" action="{{ route('ventas.anular', $venta->id) }}" method="POST" class="d-inline">
                            @csrf
                            <button type="button"
                                onclick="confirmarAnulacion('{{ $venta->id }}', '{{ $venta->serie }}-{{ $venta->numero }}')"
...
<script>
    function confirmarAnulacion(id, correlativo) {
        Swal.fire({
            ...
            html: `Vas a anular la venta <b>${correlativo}</b>.<br><br>` +
                `<div class="alert alert-warning text-left" style="font-size: 0.85rem;">` +
                `<i class="fas fa-info-circle"></i> Se generará una <b>Nota de Crédito</b> y se enviará a la cola de SUNAT automáticamente.</div>`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#6c757d',
            confirmButtonText: '<i class="fas fa-check"></i> Sí, proceder',
            cancelButtonText: 'Cancelar',
            reverseButtons: true,
            focusCancel: true
        }).then((result) => {
            if (result.isConfirmed) {
                // Mostrar estado de carga antes de recargar la página
                Swal.fire({
                    title: 'Procesando...',
                    text: 'Registrando anulación y devolviendo stock',
